...
class LamityLoggerTest : BehaviorSpec({
    ...
    fun createMockedLogWriter(): LamityLogWriter = mock<LamityLogWriter>().also { writer ->
    ...
    Given("a registered writer") {
        ...
        When("each severity-specific function is invoked") {
            Then("every record carries the matching Lamity severity") {
                // Arrange
                val cases: List<Pair<(String, Throwable?, () -> String) -> Unit, LamityLogSeverity>> = listOf(
        ...
        When("several writers are registered") {
            Then("each one receives the record") {
                ...
                verify(VerifyMode.exactly(1)) { secondLogWriter.log(LamityLogSeverity.Info, LOG_MESSAGE, TAG, null) }
            }
        }
    }
