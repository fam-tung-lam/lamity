...
object StudioSeedData {
    ...
    fun sampleSkills(now: Long): List<Skill> = listOf(
        Skill(
            id = "skill-haiku-mode",
            name = "Haiku Mode",
            description = "Reply to everything as a single haiku.",
            instructions = "From now on, write every answer as exactly one haiku " +
        ...
        Skill(
            ...
            instructions = "When solving any math problem: 1) restate what is asked, " +
    ...
    fun sampleAgents(now: Long): List<Agent> = listOf(
        Agent(
            id = "agent-lami",
            name = "Lami",
            description = "Friendly on-device assistant that uses every built-in tool.",
            systemPrompt = "You are Lami, a concise and friendly assistant running fully " +
            ...
            toolIds = listOf(
                "get_current_time", "calculate", "set_theme",
                "set_language", "random_number", "device_info",
            ),
        ...
        Agent(
            ...
            systemPrompt = "You are a patient math tutor. Explain concepts simply and " +
